 ...
 * Uses unified avatar component for consistent display
 */
 ...
 */
function renderNavigationShowcase(container) {
  container.innerHTML = '';
  ...
  const sampleHeader = createHeader({
  ...
  const progressExample = createProgressIndicator({
    current: 1,
    total: 3,
  ...
  const accordionExample = createAccordion({
    sections: [
      {
  ...
  const breadcrumbExample = createBreadcrumb({
    items: [
      { text: 'Home', href: '#' },
      { text: 'Lessons', href: '#' },
      { text: 'Create Lesson' }
...
if (typeof window !== 'undefined' && document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', () => {
    const container = document.querySelector('.navigation-showcase');
    if (container) {
